...
import { z } from "zod";
import { mealsPrisma, MealsPrisma, type GroceryItem } from "@platform/db";
...
async function findCurrentList(userId: string) {
  return mealsPrisma.groceryList.findFirst({
    where: { userId },
...
function toJson(items: GroceryItem[]): MealsPrisma.InputJsonValue {
  return items as unknown as MealsPrisma.InputJsonValue;
}
...
router.patch("/items/:itemId", requireAuth, async (req, res) => {
  ...
  const updated = await mealsPrisma.groceryList.update({
    where: { id: list.id },
