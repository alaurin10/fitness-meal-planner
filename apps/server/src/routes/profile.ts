...
const profileSchema = z.object({
  ...
  leftoverPreference: z.enum(["none", "occasional", "often"]).optional(),
  varietyStrength: z.enum(["low", "medium", "high"]).optional(),
  cuisineLikes: z.array(z.string().trim().min(1).max(40)).max(20).optional(),
  cuisineDislikes: z.array(z.string().trim().min(1).max(40)).max(20).optional(),
