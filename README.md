...
- **If in-app sign-in is rate-limited** ("Garmin is rate-limiting sign-in attempts…"): Garmin's SSO rate-limits by IP reputation, and cloud hosts like Railway share outbound IPs across many customers — so production sign-in can stay throttled indefinitely (waiting doesn't help) even though the same credentials work fine from your browser. Ongoing syncs are unaffected (they use stored tokens); only the sign-in step is blocked. Workaround: sign in from your own computer and import the tokens. In a checkout of this repo, run `pnpm --filter @app/server garmin:export-tokens` — it walks through the Garmin sign-in (including the 2FA code) from your home IP and prints a one-line token JSON. Paste that into **Connections → Garmin watch → "Import tokens from your computer"**. The tokens are verified against Garmin, then stored encrypted like a normal link; the script needs no database or env vars and never stores anything.
