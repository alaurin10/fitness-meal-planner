...
- **Sign-in works from cloud hosting.** Garmin's SSO/OAuth hosts sit behind Cloudflare, which gates on the TLS handshake fingerprint (JA3/JA4), not the User-Agent header — a plain Node client is fingerprinted as "not a browser" and 429/403'd from shared datacenter IPs, even with correct credentials. `login.ts` therefore makes its HTTP calls through [`impit`](https://github.com/apify/impit) with **Chrome impersonation**, presenting a real browser TLS fingerprint so any user can sign in from the deployed server. (Combined with the SSO *widget* flow, which sidesteps Garmin's per-account rate-limit bucket, this is what makes in-app sign-in reliable in production.)
...
- **Fallbacks if a server IP is still blocked** (rare, only if Cloudflare/Garmin hard-block the host's egress): set `GARMIN_PROXY_URL` to route sign-in through a trusted/residential proxy (no code change) — or, for a single user, run `pnpm --filter @app/server garmin:export-tokens` from your own computer to sign in from your home IP and paste the printed one-line token JSON into **Connections → Garmin watch → "Import tokens from your computer"**. Imported tokens are verified against Garmin, then stored encrypted like a normal link; the script needs no database or env vars and stores nothing.
